...
#' match_rows(msft, change, "UP{4,} DOWN{3,}", match_name=mnum)
#' }
match_rows_fast <- function(df, definitions, rx, match_name=NULL, wildcards=NULL, keep_all_rows=FALSE) {
  definitions <- rlang::enquo(definitions)
  match_name <- rlang::enquo(match_name)
  
  
  
  all_nicks <- sort(unique(dplyr::pull(df, !!definitions)))
  ...
  dplyr::group_modify(df, ~ match_partition_fast(df=.x, definitions, rx_parsed, all_nicks, match_name, keep_all_rows)) 
}
